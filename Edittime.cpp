...
int WINAPI DLLExport CreateObject(mv _far *mV, fpLevObj loPtr, LPEDATA edPtr)
{
	...
	if ( IS_COMPATIBLE(mV) )
	{
		...
		memset(edPtr->images, 0, sizeof(edPtr->images));
		edPtr->imageCount = 0;
...
void WINAPI DLLExport EditorDisplay(mv _far *mV, fpObjInfo oiPtr, fpLevObj loPtr, LPEDATA edPtr, RECT FAR *rc)
{
	...
	Surface->Rectangle(rc->left, rc->top, rc->right, rc->bottom, RGB(128, 128, 128), 1, RGB(50, 50, 50), TRUE);
    SDK->Icon->Blit(*Surface, (rc->left + rc->right) / 2 - 16, (rc->top + rc->bottom) / 2 - 16, BMODE_TRANSP, BOP_COPY, 0);
...
BOOL WINAPI DLLExport GetProperties(LPMV mV, LPEDATA edPtr, BOOL bMasterItem)
{
	...
	mvInsertProps(mV, edPtr, Properties, PROPID_TAB_DISPLAY, TRUE);
#endif // !defined(RUN_ONLY)
...
BOOL WINAPI DLLExport EditProp(LPMV mV, LPEDATA edPtr, UINT nPropID)
{
	...
	{
		TCHAR titleBuffer[MAX_IMAGES][20]{};
		for (size_t i = 0; i < MAX_IMAGES; i++)
		{
			_stprintf(titleBuffer[i], _T("Texture %d"), i);
		}
		TCHAR* titles[MAX_IMAGES + 1]{};
		for (size_t i = 0; i < MAX_IMAGES; i++)
		{
			titles[i] = &titleBuffer[i][0];
		}

		EditAnimationParams eap{};
		eap.m_dwSize = sizeof(EditAnimationParams);
		eap.m_pWindowTitle = _T("Textures");
		eap.m_nImages = edPtr->imageCount;
		eap.m_nMaxImages = MAX_IMAGES;
		eap.m_nStartIndex = 0;
		eap.m_pImages = &edPtr->images[0];
		eap.m_pImageTitles = &titles[0];
		eap.m_dwOptions = PICTEDOPT_CANBETRANSPARENT; // Define PICTEDOPT_FIXEDIMGSIZE etc. if needed
		eap.m_dwFixedWidth = 256;
		eap.m_dwFixedHeight = 256;

		if (mV->mvEditAnimation(edPtr, &eap, mV->mvHEditWin) == TRUE)
		{
			edPtr->imageCount = eap.m_nImages;
			...
			{
				Img ifo;
				GetImageInfos(mV->mvIdAppli, edPtr->images[i], &ifo);
				printf("%d = %dx%d", i, ifo.imgWidth, ifo.imgHeight);
			}
