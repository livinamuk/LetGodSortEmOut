...
void Renderer::RenderFrame()
{
  //  if (s_renderMode == RENDER_MODE_FUSION)
    ...
    */
    ...
    //Scene::s_runtime_lights["PlayerLight"].m_position = glm::vec2(Input::s_mouseWorldX, Input::s_mouseWorldY);
...
void Renderer::LightingPass()
{
    ...
    glBindVertexArray(s_lightVAO);
    glBindBuffer(GL_ARRAY_BUFFER, s_lightVBO);
    glEnableVertexAttribArray(0);
    glVertexAttribPointer(0, 2, GL_FLOAT, GL_FALSE, sizeof(glm::vec2), (void*)0);
   ...
   /* for (auto& it : Scene::s_runtime_lights)
    {
    ...
    */
    ...
    glDisable(GL_BLEND);
    DownScale(s_gBuffer.ID, GL_COLOR_ATTACHMENT2, 3);
    glEnable(GL_BLEND);
...
void Renderer::RenderFinalImage(Shader* shader)
{
    ...
    if (s_renderLOS)
        shader->setBool("u_renderLOS", true);
    else
        shader->setBool("u_renderLOS", false);

    if (s_renderLIGHTING)
        shader->setBool("u_renderLighting", true);
    else
        shader->setBool("u_renderLighting", false);
...
{
    if (s_renderMode == RENDER_MODE_FUSION_WORLD_ONLY) {
        s_renderMode = RENDER_MODE_FUSION_WORLD_PLUS_LIGHTING;
           return;
    }
    if (s_renderMode == RENDER_MODE_FUSION_WORLD_PLUS_LIGHTING) {
        s_renderMode = RENDER_MODE_FUSION_WORLD_ONLY;
        return;
    }
}
